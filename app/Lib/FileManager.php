...
class FileManager {
    ...
    protected function uploadImage() {
        ...
        $image   = $manager->read($this->file->get());

        // Change extension to webp
        $filenameData   = pathinfo($this->filename);
        $this->filename = $filenameData['filename'] . '.webp';
        ...
        if ($this->size) {
            ...
            // Resize tối đa 1800px chiều rộng, giữ tỷ lệ gốc và làm nét
            $image->scaleDown(width: 1800)->sharpen(5);
        }

        // save the image as webp with 90% quality
        $image->toWebp(90)->save($this->path . '/' . $this->filename);

        // save the image as thumbnail version
        if ($this->thumb) {
            if ($this->old) {
                // Remove old thumbnail (might have different extension)
                $oldData = pathinfo($this->old);
                $oldThumbPath = $this->path . '/thumb_' . $oldData['filename'] . '.webp';
                if (file_exists($oldThumbPath)) {
                    @unlink($oldThumbPath);
                }
                // Also try removing the exact old filename as thumb if it was different
                $this->removeFile($this->path . '/thumb_' . $this->old);
            }
            $thumb = explode('x', $this->thumb);
            $manager->read($this->file->get())->resize($thumb[0], $thumb[1])->toWebp(80)->save($this->path . '/thumb_' . $this->filename);
        }
    }
    ...
    public function removeFile($path = null) {
        ...
        if (file_exists($path) && is_file($path)) {
            ...
            // Fallback: Try with .webp extension if original file not found
