...
{
    $name = $type ? "/logo_$type.webp" : '/logo.webp';
...
{
    return getImage(getFilePath('logoIcon') . '/favicon.webp');
}
...
function showAmount($amount, $decimal = 2, $separate = true, $exceptZeros = false, $currencyFormat = true, $kFormat = false)
{
    $separator = '';
    if ($separate && !$kFormat) {
        $separator = ',';
    ...
    if ($kFormat) {
        $printAmount = ($printAmount / 1000) . 'K';
    }
    if ($exceptZeros) {
    ...
    if ($currencyFormat) {
        if (gs('currency_format') == Status::CUR_BOTH) {
            $printAmount = gs('cur_sym') . $printAmount . ' ' . __(gs('cur_text'));
        } else if (gs('currency_format') == Status::CUR_TEXT) {
            $printAmount = $printAmount . ' ' . __(gs('cur_text'));
        } else {
            $printAmount = gs('cur_sym') . $printAmount;
        }
    }
    return $printAmount;
}
...
{
    $group = \App\Models\MenuGroup::active()->where('location', $location)->first();
    if (!$group) {
        return [];
    }
    ...
    $menuItems = \App\Models\MenuItem::active()
        ...
        ->where('parent_id', 0)
        ->with(['translations', 'children' => function ($query) {
            $query->active()->with('translations');
        }])
        ->orderBy('order', 'asc')
        ->get();

    return build_menu_tree($menuItems);
}
...
function build_menu_tree($items)
{
    ...
    foreach ($items as $item) {
        $tree[] = [
            'id'       => $item->id,
            'title'    => $item->t->title ?? $item->title,
            'url'      => $item->url,
            'children' => $item->children->count() > 0 ? build_menu_tree($item->children) : [],
        ];
    }
    return $tree;
}
...
{
    $clean = '';
    $cleanPath = str_replace(url('/'), '', $image);
...
function styleSelectedWord($htmlContent, $length = 1, $isArray = false)
{
    $words = explode(' ', trans($htmlContent));
    if (count($words) > 0) {
        $styleWords = array_splice($words, -$length);
        if ($length > 1) {
            $styledWord = '<span class="text--base d-inline">' . implode(' ', array_slice($styleWords, -$length)) . '</span>';
            if ($isArray) {
                return [implode(' ', $words), $styledWord];
            } else {
                array_push($words, $styledWord);
                return implode(' ', $words);
            }
        }
        $styledWord = '<span class="text--base">' . $styleWords[0] . '</span>';
        array_push($words, $styledWord);
        return implode(' ', $words);
    }
    return $words;
...
function authCheck()
{
    return auth()->check() || auth()->guard('user')->check() ? true : false;
}

function getProfileImage($filename, $path = 'userProfile')
{
    if ($filename) {
        return getImage(getFilePath($path) . '/' . $filename, getFileSize($path));
    } else {
        return asset('assets/images/userdp.png');
    }
}
