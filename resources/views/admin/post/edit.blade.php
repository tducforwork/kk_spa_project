                            ...
                            <div class="col-md-4">
                                ...
                                @if($post->images->count() > 0)
                                    <div class="card bg--light mt-3">
                                        <div class="card-header">@lang('Existing Gallery')</div>
                                        <div class="card-body">
                                            <div class="row g-2">
                                                @foreach($post->images as $img)
                                                    <div class="col-md-4 col-sm-6 text-center">
                                                        <div class="border rounded p-1 bg-white">
                                                            <img src="{{ getImage(getFilePath('post') . '/' . $img->image) }}" class="w-100 rounded mb-1" style="height: 80px; object-fit: cover;">
                                                            <button type="button" class="btn btn-sm btn-outline-danger w-100 confirmationBtn" 
                                                                data-question="@lang('Are you sure to remove this image?')" 
                                                                data-action="{{ route('admin.post.image.remove', $img->id) }}">
                                            ...
                                            @foreach($languages as $lang)
                                                ...
                                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="{{ $lang->code }}" role="tabpanel" aria-labelledby="{{ $lang->code }}-tab">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control name-input" name="translations[{{ $lang->code }}][name]" data-lang="{{ $lang->code }}" value="{{ old('translations.' . $lang->code . '.name', @$translation->name ?: ($lang->code == 'en' ? @$post->name : '')) }}" required>
                                                    </div>
                                                    <div class="form-group">
                                                        <label class="required">@lang('Slug') ({{ strtoupper($lang->code) }})</label>
                                                        <input type="text" class="form-control slug-input" name="translations[{{ $lang->code }}][slug]" data-lang="{{ $lang->code }}" value="{{ old('translations.' . $lang->code . '.slug', @$translation->slug ?: ($lang->code == 'en' ? @$post->slug : '')) }}" required>
                                                    </div>
                                                    <div class="form-group">
                                                        <label>@lang('Short Description')</label>
                                                        <textarea class="form-control" name="translations[{{ $lang->code }}][description]">{{ old('translations.' . $lang->code . '.description', @$translation->description ?: ($lang->code == 'en' ? @$post->description : '')) }}</textarea>
                                                    </div>
                                                    <div class="form-group">
                                                        <label>@lang('Content')</label>
                                                        <textarea class="form-control nicEdit" name="translations[{{ $lang->code }}][content]">{{ old('translations.' . $lang->code . '.content', @$translation->content ?: ($lang->code == 'en' ? @$post->content : '')) }}</textarea>
                                                    </div>
