                                        ...
                                        @if ($k != 'images')
                                            @php
                                                ...
                                                if (is_object($item) || is_array($item)) {
                                                    $item = (object) $item;
                                                    $isI18n = isset($item->i18n) ? $item->i18n : true;
                                                    $fieldType = $item->type;
                                                }
                                            @endphp
                                                                ...
                                                                @foreach ($languages as $lang)
                                                                    <div class="form-group">
                                                                        ...
                                                                        <textarea rows="10" class="form-control @if ($fieldType == 'textarea-nic') nicEdit @endif"
                                                                            name="{{ $k }}_{{ $lang->code }}">{{ @$content->data_values->{$k . '_' . $lang->code} ?? @$content->data_values->$k }}</textarea>
                                                                    </div>
                                                                @endforeach
                                                            @else
                                                                <div class="form-group">
                                                        ...
                                                        <div class="card-body">
                                                            @if ($isI18n)
                                                                @foreach ($languages as $lang)
                                                                    <div class="form-group">
                                                                        <label>{{ __(keyToTitle($k)) }} - {{ __($lang->name) }}</label>
                                                                        <input type="text" class="form-control" name="{{ $k }}_{{ $lang->code }}"
                                                                            value="{{ @$content->data_values->{$k . '_' . $lang->code} ?? @$content->data_values->$k }}"
                                                                            required>
                                                                    </div>
                                                                @endforeach
                                                            @else
                                            ...
                                            @foreach ($section->element as $k => $type)
                                                @if ($k != 'modal' && $k != 'seo')
                                                    @if ($type == 'text' || $type == 'icon')
                                                        ...
                                                        @else
                                                            <td>{{ __(@$data->data_values->{$k . '_en'} ?? (@$data->data_values->{$k . '_vi'} ?? @$data->data_values->$k)) }}</td>
                                                        @endif
                                                    @elseif($k == 'select')
                                                        @php
                                        ...
                                        <div class="card border--primary mt-3">
                                            ...
                                            <div class="card-body">
                                                @foreach ($languages as $lang)
                                                    <div class="form-group">
                                                        <label>{{ __(keyToTitle($k)) }} - {{ __($lang->name) }}</label>
                                                        <textarea rows="4" class="form-control @if($type == 'textarea-nic') nicEdit @endif" 
                                                            name="{{ $k }}_{{ $lang->code }}"></textarea>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
                                        ...
                                        <div class="card border--primary mt-3">
                                            ...
                                            <div class="card-body">
                                                @foreach ($languages as $lang)
                                                    <div class="form-group">
                                                        <label>{{ __(keyToTitle($k)) }} - {{ __($lang->name) }}</label>
                                                        <input type="text" class="form-control" name="{{ $k }}_{{ $lang->code }}" required>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
                            ...
                            @foreach ($section->element as $k => $type)
                                @if ($k != 'modal')
                                    ...
                                    @elseif($type == 'textarea' || $type == 'textarea-nic')
                                        <div class="card border--primary mt-3">
                                            <div class="card-header bg--primary">
                                                <h5 class="text-white">{{ keyToTitle($k) }}</h5>
                                            </div>
                                            <div class="card-body">
                                                @foreach ($languages as $lang)
                                                    <div class="form-group">
                                                        <label>{{ keyToTitle($k) }} - {{ __($lang->name) }}</label>
                                                        <textarea rows="4" class="form-control @if($type == 'textarea-nic') nicEdit @endif" 
                                                            name="{{ $k }}_{{ $lang->code }}" required></textarea>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
        ...
        (function($) {
            ...
            $(document).on('click', '.updateBtn', function() {
                ...
                $.each(obj, function(index, value) {
                    ...
                    // Also try to populate localized fields if index doesn't have suffix
                    @foreach($languages as $lang)
                        modal.find('[name=' + index + '_{{ $lang->code }}]').val(obj[index + '_{{ $lang->code }}'] || value);
                    @endforeach
                });
                modal.modal('show');
            });
