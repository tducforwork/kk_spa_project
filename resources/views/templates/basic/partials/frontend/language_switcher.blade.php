@php
    ...
    $currentLang = $languages->where('code', app()->getLocale())->first();

    // Auto-translate languages list
    $autoLangs = [
        ['code' => 'ko', 'name' => 'Korean', 'flag' => 'https://flagcdn.com/w40/kr.png'],
        ['code' => 'zh-CN', 'name' => 'Chinese (S)', 'flag' => 'https://flagcdn.com/w40/cn.png'],
        ['code' => 'zh-TW', 'name' => 'Chinese (T)', 'flag' => 'https://flagcdn.com/w40/tw.png'],
...
<li class="nav-item lang-item" id="languageSwitcher">
    ...
    <div class="language-dropdown">
        ...
        <div class="dropdown-header mt-2">@lang('Smart Translate')</div>
        <div class="language-group">
            @foreach ($autoLangs as $auto)
                <a href="javascript:void(0)" class="dropdown-item auto-translate-btn" data-lang="{{ $auto['code'] }}">
                    <img src="{{ $auto['flag'] }}" alt="{{ $auto['name'] }}">
                    <span>{{ __($auto['name']) }}</span>
                </a>
            @endforeach
        </div>
    </div>
